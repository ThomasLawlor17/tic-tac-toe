...
const changeWinCellColor = (player: "X" | "O") => {
	let winCells: { a: string; b: string; c: string } = { a: "", b: "", c: "" };
	if (
		data[0][0] === data[0][1] &&
		data[0][1] === data[0][2] &&
		data[0][2] === player
...
const handleCellClick = (e: Event) => {
	...
	const cell: HTMLElement = <HTMLElement>e.target;
...
const move = (cell: HTMLElement, x: number, y: number) => {
	...
	if (result !== "T") {
		...
		roundWinnerPlayerIdentifier.innerHTML =
			(mode === 1 && result === "X") || (mode === 2 && result === "O")
				...
				? "OH NO, YOU LOST..."
				: `PLAYER ${
						(mode === 3 && result === "X") || (mode === 4 && result === "O")
							? "1"
							: "2"
				  } WINS!`;
...
cells.forEach((cell) =>
	cell.addEventListener("mouseleave", (e) => cellHover(e))
);
